...
namespace JustChat.Application.Models.Specifications.Base
{
         ...
         where T : class
    {
        ...
        public override BaseSpecification<T> AddCriteria(Expression<Func<T, bool>> criteria)
        {
            return _spec.AddCriteria(criteria);
        }

        public override BaseSpecification<T> AddInclude(Expression<Func<T, object>> includeExpression)
        {
            return _spec.AddInclude(includeExpression);
        }

        public override OrderedSpecification<T> ApplyOrderBy(Expression<Func<T, object>> orderByExpression)
        {
            return _spec.ApplyOrderBy(orderByExpression);
        }

        public override OrderedSpecification<T> ApplyOrderByDescending(Expression<Func<T, object>> orderByDescendingExpression)
        {
